...
import type { GitHubApi, CurationStore, CurationRecord, RegistryClient, Http, Clock, RepoMeta, CacheStore, RepoCache } from "@/scripts/pipeline/ports";
...
const rec = (fn: string): CurationRecord => ({ full_name: fn, name: fn.split("/")[1], kind: "mcp", category: "developer", tags: ["a"], description_en: "A tool.", description_zh: "工具。", long_en: "L.", long_zh: "长。", install_spec: {}, sec_note_en: "Reviewed.", sec_note_zh: "已审核。" });
const store: CurationStore = { get: (fn) => rec(fn), getForRepo: (fn) => [rec(fn)], all: () => [] };
...
describe("incremental run", () => {
  it("reuses cached entries without calling curate when READMEs are unchanged", async () => {
    ...
    for (let i = 0; i < 8; i++) seed[`acme/foo${i}`] = { etag: "e", readme_hash: contentHashReadme(README), entries: [curatedFixture(`acme/foo${i}`)] };
