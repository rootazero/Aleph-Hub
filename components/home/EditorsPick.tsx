...
import { editorsPick, formatStars, slugForEntry } from "@/lib/catalog";

// Mockup lines 56-67. The "editor's pick" is the highest-starred entry.
export function EditorsPick() {
  const { lang } = useLang();
  const t = STRINGS[lang];
  const pick = editorsPick();
  const desc = lang === "zh" ? pick.description_zh : pick.description_en;
  return (
    <Link href={`/e/${slugForEntry(pick)}`} style={{ display: "block", textDecoration: "none", color: "inherit", background: "var(--panel)", border: "1px solid var(--hair)", borderRadius: 3, overflow: "hidden", boxShadow: "0 12px 40px rgba(36,28,22,.10)" }}>
